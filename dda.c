...
void update_current_position() {
  ...
  static const axes_uint32_t_F steps_per_mm_F = {
    ...
    for (i = X; i < AXIS_COUNT; i++) {
      current_position.axis[i] = dda->endpoint.axis[i] -
          ...
          // Should be: move_state.steps[i] * 1000000 / steps_per_m_F[i])
          ...
          ((move_state.steps[i] * 1000) / steps_per_mm_F[i]);
    }

    if (dda->endpoint.e_relative)
      current_position.axis[E] =
          (move_state.steps[E] * 1000) / steps_per_mm_F[E];
